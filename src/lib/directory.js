...
function isFile(filePath) {
  try {
    // Esta función devuelve un objeto que contiene información sobre el archivo, como tamaño,
    // permisos, etc.
    const stats = fs.statSync(filePath);
    // Para ello, utilizamos el método `stats.isFile()` que devuelve `true` si es un archivo regular
...
function readDirectoryRecursively(directorioPath) {
  ...
  // Utilizamos la función `fs.readdirSync`
  //  para obtener los contenidos de un directorio de forma síncrona.
  // Esta función toma como argumento la ruta del directorio y devuelve un array
  // con los nombres de los archivos y directorios contenidos en él.
  ...
  let arrayPathFiles = [];
  files.forEach((file) => {
    // Utilizamos la función `path.join` para unir el directorio base (`directorioPath`)
    // con el nombre del archivo o directorio (`file`)
    // y obtener la ruta completa del archivo o directorio.
